...
void custom_agg_renderer<T0, T1>::process(polygon_symbolizer const& sym,
    ...
    render_polygon_symbolizer<vertex_converter_type>(
      ...
      *ras_ptr,
      [&](color const& fill, double opacity) {          
          ...
          if (this->polygon_symbolizer_callback)
          {
              this->polygon_symbolizer_callback(&buf, fill, opacity);
          }
          else
          {
              unsigned r = fill.red();
              ...
              pixf.comp_op(static_cast<agg::comp_op_e>(
                get<composite_mode_e>(sym, keys::comp_op, feature, common_.vars_, src_over)));
...
template void
  custom_agg_renderer<image_rgba8>::process(polygon_symbolizer const&, mapnik::feature_impl&, proj_transform const&);
