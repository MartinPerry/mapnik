...
custom_agg_renderer<T0, T1>::custom_agg_renderer(Map const& m, T0& pixmap, double scale_factor, unsigned offset_x, unsigned offset_y)
    : feature_style_processor<custom_agg_renderer>(m, scale_factor)
    , buffers_()
...
template<typename T0, typename T1>
custom_agg_renderer<T0, T1>::custom_agg_renderer(Map const& m,
                                   request const& req,
...
template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::setup(Map const& m, buffer_type& pixmap)
{
    buffers_.emplace(pixmap);

    mapnik::set_premultiplied_alpha(pixmap, true);
    std::optional<color> const& bg = m.background();
    if (bg)
    {
        if (bg->alpha() < 255)
        {
    ...
    std::optional<std::string> const& image_filename = m.background_image();
    if (image_filename)
    {
    ...
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: Scale=" << m.scale();
}

template<typename T0, typename T1>
custom_agg_renderer<T0, T1>::~custom_agg_renderer()
{}

template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::start_map_processing(Map const& map)
{
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: Start map processing bbox=" << map.get_current_extent();
    ras_ptr->clip_box(0, 0, common_.width_, common_.height_);
}

template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::end_map_processing(Map const& map)
{
    mapnik::demultiply_alpha(buffers_.top().get());
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: End map processing";
}

template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::start_layer_processing(layer const& lay, box2d<double> const& query_extent)
{
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: Start processing layer=" << lay.name();
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: -- datasource=" << lay.datasource().get();
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: -- query_extent=" << query_extent;
    ...
    std::optional<box2d<double>> const& maximum_extent = lay.maximum_extent();
    if (maximum_extent)
    {
...
template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::end_layer_processing(layer const& lyr)
{
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: End layer processing";
...
template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::start_style_processing(feature_type_style const& st)
{
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: Start processing style";
...
template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::end_style_processing(feature_type_style const& st)
{
    buffer_type& current_buffer = buffers_.top().get();
    buffers_.pop();
    buffer_type& previous_buffer = buffers_.top().get();
    ...
    MAPNIK_LOG_DEBUG(custom_agg_renderer) << "custom_agg_renderer: End processing style";
}
...
template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::render_marker(pixel_position const& pos,
                                         marker const& marker,
                                         agg::trans_affine const& tr,
                                         double opacity,
                                         composite_mode_e comp_op)
...
template<typename T0, typename T1>
bool custom_agg_renderer<T0, T1>::painted()
{
    return buffers_.top().get().painted();
}

template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::painted(bool painted)
{
    buffers_.top().get().painted(painted);
}

template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::debug_draw_box(box2d<double> const& box, double x, double y, double angle)
{
    buffer_type& current_buffer = buffers_.top().get();
...
template<typename T0, typename T1>
template<typename R>
void custom_agg_renderer<T0, T1>::debug_draw_box(R& buf, box2d<double> const& box, double x, double y, double angle)
{
...
template<typename T0, typename T1>
void custom_agg_renderer<T0, T1>::draw_geo_extent(box2d<double> const& extent, mapnik::color const& color)
{
...
template class custom_agg_renderer<image_rgba8>;
template void custom_agg_renderer<image_rgba8>::debug_draw_box<agg::rendering_buffer>(agg::rendering_buffer& buf,
